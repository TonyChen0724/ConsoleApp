...
uint32_t *GetGPMFPayload(uint32_t *lastpayload, uint32_t index)
{
	...
	{
		GPMFbuffer = (uint32_t *)realloc(lastpayload, metasizes[index]);
...
#define NESTSIZE(x) { int32_t i = nest; while (i > 0 && nestsize[i] > 0) { nestsize[i] -= x; if(nestsize[i]>=0 && nestsize[i] <= 8) { nestsize[i]=0; nest--; } i--; } }
