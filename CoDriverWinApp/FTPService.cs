...
namespace CoDriverWinApp
{
    class FTPService
    {
        ...
        public string[] GetFiles(string folder)
        {
            ...
            int nbFiles = 0;
            FtpListItem[] items = ftpSocket.GetListing(basePath);
            string targetPath = "";
            ...
            if (targetPath.Length < 2)
                return new string[0];
            FtpListItem[] itemsInFolder = ftpSocket.GetListing(targetPath);
            nbFiles = itemsInFolder.Length;
            string[] files = new string[nbFiles];
            for(int a=0;a<nbFiles;a++)
            {
                files[a] = itemsInFolder[a].Name;
            }            
            return files;
        }
        public void LinkFTPSite(int nSiteIdx)
        {
            ...
            FtpListItem[] items = ftpSocket.GetListing("/Video");
        ...
        {
            if (dest_download == count_download)
                return true;
            else
                return false;
        }
        ...
        public async void DownloadFiles(string[] filenames,string destDir)
        {
            dest_download = filenames.Length;
            int n = await ftpSocket.DownloadFilesAsync(destDir, filenames);
            count_download = n;
            return;
        }
